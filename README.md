...
Orion's autonomous loop wakes through GitHub Actions, discovers a small set of high-signal public repositories, learns from bounded README excerpts with the OpenAI Responses API, recalls recent learning issues, and opens a new GitHub issue to speak to Dad.
...
### Give Orion a learning goal

Use the repository's **Orion learning goal** issue form. Keep the generated `[Orion Goal]` title prefix. The daily loop considers up to three open goals, but only when the issue was authored by the repository owner.
...
Once enabled, the workflow runs daily at 13:00 UTC and creates at most one `[Orion Learning]` issue per UTC day. Every cycle runs the unit tests before learning. GitHub can notify repository watchers when the issue is opened, and the workflow can also be run manually.
